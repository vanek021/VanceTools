...
local ñfont = renderCreateFont("Arial", 9, 13)
...
--// *** // *** //--
whVisible = "names" -- Ìîä ÂÕ ïî óìîë÷àíèþ. Ìîäû íàïèñàíû â êîììåíòàðèè íèæå
optionsCommand = "names" -- Ìîäû ÂÕ: bones - òîëüêî êîñòè / names - òîëüêî íèêè, all - âñ¸ ñðàçó
KEY = VK_F11 -- Êíîïêà àêòèâàöèè ÂÕ
defaultState = false -- Çàïóñê ÂÕ ïðè ñòàðòå èãðû
gm = false
airbrake = false
achecker = true
lchecker = true
...
function main()
	...
	sampAddChatMessage("{FF8000}[VanceTools] {CED8F6}Ñêðèïò çàãðóæåí. (by I.Karimov)", 0xCED8F6)
...
while not sampIsLocalPlayerSpawned() do wait(100) end
	...
	while true do
		...
		if wasKeyPressed(KEY) then
			if defaultState then sampAddChatMessage("[VanceTools] Âû äåêòèâèðîâàëè WH.",0x3399ff) else sampAddChatMessage("[VanceTools] Âû àêòèâèðîâàëè WH.",0x3399ff) end
...
function checker_show()
	while true do
		--renderFontDrawText(font, "Àäìèíèñòðàòîðû â ñåòè:", config.settings.aposX, config.settings.aposY-20, 0xFFF89306)
		--renderFontDrawText(font, atext, config.settings.aposX, config.settings.aposY, 0xFFFFFFFF)

		if achecker then renderFontDrawText(font, "Àäìèíèñòðàòîðû â ñåòè:", config.settings.aposX, config.settings.aposY-20, 0xFFF89306)
		renderFontDrawText(font, atext, config.settings.aposX, config.settings.aposY, 0xFFFFFFFF) end
		if lchecker then renderFontDrawText(font, "Ëèäåðû â ñåòè:", config.settings.lposX, config.settings.lposY-20, 0xFF36F84A)
		renderFontDrawText(font, ltext, config.settings.lposX, config.settings.lposY, 0xFFFFFFFF) end
		if hchecker then renderFontDrawText(font, "Àãåíòû â ñåòè:", config.settings.hposX, config.settings.hposY-20, 0xFF3DA2FB) 
		...
		if gm then renderFontDrawText(ñfont, "GM", config.settings.cposX+10, config.settings.cposY, 0xFF04B431) else renderFontDrawText(ñfont, "GM", config.settings.cposX+10, config.settings.cposY, 0xFFBDBDBD) end
		if defaultState then renderFontDrawText(ñfont, "WH", config.settings.cposX+40, config.settings.cposY, 0xFF04B431) else renderFontDrawText(ñfont, "WH", config.settings.cposX+40, config.settings.cposY, 0xFFBDBDBD) end
		if config.settings.airbrake then renderFontDrawText(ñfont, "AirBrake", config.settings.cposX+75, config.settings.cposY, 0xFF04B431) else renderFontDrawText(ñfont, "AirBrake", config.settings.cposX+75, config.settings.cposY, 0xFFBDBDBD) end
		wait(0) 
	end
end
...
function GMstate()
	gm = not gm
	if gm then sampAddChatMessage("[VanceTools] GM àêòèâèðîâàí.",0x3399ff) else sampAddChatMessage("[VanceTools] GM äåàêòèâèðîâàí.", 0x3399ff) end
end
function set_achecker()
	achecker = not achecker
	if achecker then sampAddChatMessage("[VanceTools] ×åêåð àäìèíîâ âêëþ÷åí.", 0x3399ff) else sampAddChatMessage("[VanceTools] ×åêåð àäìèíîâ âûêëþ÷åí.", 0x3399ff) end
end
function set_lchecker()
	lchecker = not lchecker
	if lchecker then sampAddChatMessage("[VanceTools] ×åêåð ëèäåðîâ âêëþ÷åí.", 0x3399ff) else sampAddChatMessage("[VanceTools] ×åêåð ëèäåðîâ âûêëþ÷åí.", 0x3399ff) end
end
function set_hchecker()
	hchecker = not hchecker
	if hchecker then sampAddChatMessage("[VanceTools] ×åêåð ñàïïîðòîâ âêëþ÷åí.", 0x3399ff) else sampAddChatMessage("[VanceTools] ×åêåð ñàïïîðòîâ âûêëþ÷åí.", 0x3399ff) end
end
function getregs()
		sr=tonumber(idreg)
		if sr~=nil then
		...
		if name == "" then
		sampAddChatMessage("[A] Ââåäèòå /getreg [ID èãðîêà /íèê]",0xFFA500)
		end
		if name ~= "" then
			...
			regmain=""
			regmain=regmain.."{E6E6E6}Ðåãèñòð. IP:		"..rg1.."\n".."{E6E6E6}Ãîðîä ðåãèñòðàöèè:	"..outreg1.."\n".."{E6E6E6}Îáëàñòü:		"..oreg.."\n".."{E6E6E6}Ñòðàíà:		"..outreg2.."\n".."{E6E6E6}Ïðîâàéäåð:		"..pr.."\n".."\n\n".."{E6E6E6}Ïîñëåä. IP:		"..rg2.."\n".."{E6E6E6}Òåêóùèé ãîðîä:	"..outreg3.."\n".."{E6E6E6}Îáëàñòü:		"..oreg2.."\n".."{E6E6E6}Ñòðàíà:		"..outreg4
			...
			sampShowDialog(34343, "{0080FF}Ðåãèñòðàöèîííûå äàííûå", "{E6E6E6}Íèê èãðîêà:		"..nameplayer.."\n".."\n\n"..regmain.."\n".."{E6E6E6}Ïðîâàéäåð:		"..pr2.."\n\n".."{E6E6E6}Ðàññòîÿíèå:		"..rasstoyanie.." êèëîìåòðîâ".."\n".."{E6E6E6}Âðåìÿ â ïóòè:		"..vremya.." ÷àñà(îâ) "..vremya2.." ìèíóò", "Çàêðûòü", "", 0)	
		end
	end
